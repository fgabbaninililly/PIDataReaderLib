...
namespace PIDataReaderLib {
	public class PIAFReader : PIReaderInterface {
		...
		*/
		...
		/*
		public PIData Read(string tagListCsvString, string phaseTagListCsvString, DateTime startTime, DateTime endTime, AFBoundaryType boundaryType) {
			SortedSet<string> tagNameList = new SortedSet<string>(tagListCsvString.Split(','));
			SortedSet<string> phaseTagNameList = new SortedSet<string>(phaseTagListCsvString.Split(','));
			List<Tag> tagList = ReadTags(tagNameList, startTime, endTime, boundaryType, false);
			List<Tag> phaseTagList = ReadTags(phaseTagNameList, startTime, endTime, boundaryType, true);
		...
		*/
		...
		/*
		[Obsolete(@"public PIData Read(string, DateTime, DateTime, AFBoundaryType) is deprecated, please use 
			public PIData Read(string, string, DateTime, DateTime, AFBoundaryType) instead.")]
		public PIData Read(string tagListString, DateTime startTime, DateTime endTime, AFBoundaryType boundaryType) {

			string[] tags = tagListString.Split(',');

			IList<PIPoint> points = OSIsoft.AF.PI.PIPoint.FindPIPoints(piServer, tags);
			dynamic pointList = new PIPointList(points);
